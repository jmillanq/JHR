" ATP project vim script: Mon Sep 07, 2015 at 02:45 PM +0100.

let b:atp_MainFile = 'DrugsGunsBabies.tex'
let g:atp_mapNn = 0
let b:atp_autex = 1
let b:atp_TexCompiler = 'pdflatex'
let b:atp_TexOptions = '-synctex=1'
let b:atp_TexFlavor = 'tex'
let b:atp_auruns = '1'
let b:atp_ReloadOnError = '1'
let b:atp_OutDir = '/Volumes/JMILLAN/WiP/DrugsGunsBabies/Documents/Drafts/JDev'
let b:atp_OpenViewer = '1'
let b:atp_XpdfServer = 'DrugsGunsBabies'
let b:atp_Viewer = 'open'
let b:TreeOfFiles = {}
let b:ListOfFiles = []
let b:TypeDict = {}
let b:LevelDict = {}
let b:atp_BibCompiler = 'bibtex'
let b:atp_StarEnvDefault = ''
let b:atp_StarMathEnvDefault = ''
let b:atp_updatetime_insert = 4000
let b:atp_updatetime_normal = 2000
let b:atp_LocalCommands = ['\sym{']
let b:atp_LocalEnvironments = ['prop']
let b:atp_LocalColors = []
